...
class Branched(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(3, 144, kernel_size = (6, 6, 6), stride=(3,3,3)),
            nn.Flatten(start_dim=1),
            nn.Linear(in_features=85349376, out_features=68),
        )

    def forward(self, x):
        """Forward pass"""
        x2 = self.conv(x)
        #print("X and x2 shape: {}, {}".format(x.shape, x2.shape))
        return x, x2

class MeshNetOld(nn.Module):
    """Configurable MeshNet from https://arxiv.org/pdf/1612.00940.pdf"""
    ...
    def __init__(self, n_channels, n_classes, large=True, dropout_p=0):
        ...
        self.outconv = nn.Conv3d(**params[-1])
        self.model = nn.Sequential(*layers) #, Branched())
...
class MeshNet(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, large=True, dropout_p=0, n_filters=False, grad_checkpoint=True):
        """Init"""
        if large:
            params = MeshNet_68_kwargs
        ...
        if n_filters:
            for param in params:
                param['in_channels'] = n_filters
                param['out_channels'] = n_filters
        params[0]["in_channels"] = n_channels
        params[-1]["out_channels"] = n_classes
        layers = [
            conv_w_bn_before_act(dropout_p=dropout_p, **block_kwargs)
        ...
        self.output_conv = nn.Conv3d(**params[-1])
        self.model = nn.Sequential(*layers)
        init_weights(self.model,)
        init_weights(self.output_conv,)
    ...
    def forward(self, x):
        ...
        if self.grad_checkpoint:
            segments = 6
            feature_map = checkpoint_sequential(self.model, segments, x)
        y_hat = self.output_conv(feature_map)
        return y_hat

class MeshNet_2_Task(MeshNet):
    def __init__(self, n_channels, n_classes, large=True, dropout_p=0, n_filters=False, n_estimates=68):

        super(MeshNet_2_Task, self).__init__(
            n_channels=n_channels, n_classes=n_classes, large=large, n_filters=n_filters)
        self.n_estimates = n_estimates
        self.avgpool = nn.AdaptiveAvgPool3d((1))
        if n_filters:
            linear_in_features = n_filters
        else:
            linear_in_features = 32

        self.output_fc = nn.Linear(21, n_estimates)

    def forward(self, x):
        """Forward pass"""
        if self.grad_checkpoint:
            feature_map = checkpoint_sequential(self.model, len(self.model), x, preserve_rng_state = False)
        y_hat = self.output_conv(feature_map)
        y_hat_2 = self.avgpool(feature_map)
        y_hat_2 = torch.flatten(y_hat_2)
        y_hat_2 = self.output_fc(y_hat_2)
        return [y_hat, y_hat_2]