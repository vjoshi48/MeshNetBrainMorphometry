...
class BrainDataset(Dataset):
    ...
    def __init__(
        ...
        gray_matter_key: str = "gray_matter"
    ):
        """
        Args:
        ...
        self.dict_transform = (
            dict_transform if dict_transform is not None else self.identity
        )
        self.input_key = input_key
        self.segmentation = segmentation_key
        self.gray_matter = gray_matter_key
    ...
    def __getitem__(self, index: Union[int, list]) -> Any:
        ...
        for i, subj in enumerate(subject_ids):
            batch_dict = self.__crop__(subj_data_dict[subj], coords_list[i])
            batch_dict['gray_matter'] = subj_data_dict[subj]['gray_matter']
    ...
    def __crop__(self, dict_, coords):
        ...
        output[self.segmentation] = output_labels.squeeze().astype(np.int64)
